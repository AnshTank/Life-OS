...
import { Clock, CheckCircle2, Target, Calendar, DollarSign, AlertTriangle, RefreshCw, Plus, Github, ExternalLink } from 'lucide-react';
      ...
      <div className="bg-white border border-[#e8dac0] rounded-xl p-4">
        ...
        <div className="space-y-2 mb-3">
          {milestones.length === 0 ? <p className="font-kalam text-xs text-slate-400 py-2 italic">No milestones yet — add key goals below</p> : (
            milestones.map(ms => (
              <div key={ms.id} className="flex items-center gap-3 p-2.5 bg-[#f9f7f4] border border-[#e8dac0] rounded-lg cursor-pointer hover:shadow-sm transition-all" onClick={() => toggleMilestone(ms.id)}>
                ...
                {ms.target && <span className="font-kalam text-[10px] text-slate-400 shrink-0">{ms.target}</span>}
              </div>
            ))
          )}
        </div>
