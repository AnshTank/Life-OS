  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
        ...
        {[
          { label: 'Total Entries', count: stats.total, icon: BookOpen, iconBg: '#f5f0e6', iconBorder: '#d9b896', iconColor: '#a88a5a' },
          { label: 'Journals',      count: stats.journals, icon: BookOpen, iconBg: '#e8eef3', iconBorder: '#7a9eb8', iconColor: '#5a7a94' },
          { label: 'Notes',         count: stats.notes, icon: FileText, iconBg: '#e8f0e9', iconBorder: '#8ab896', iconColor: '#5a9468' },
          { label: 'Reminders',     count: stats.reminders, icon: Bell, iconBg: '#fef9e6', iconBorder: '#e0d4a0', iconColor: '#a88a5a' },
        ].map((s, i) => {
          const Icon = s.icon;
          return (
            <div key={i} className="journal-card">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-kalam text-sm text-[#5a5a5a]">{s.label}</p>
                  <p className="font-caveat text-3xl">{s.count}</p>
                </div>
                ...
                {pagedEntries.length === 0 ? (
                  ...
                  return (
                    <motion.div key={entry.id}
                      ...
                      className="flex items-start gap-3 p-3 rounded-lg bg-[#f9f7f4] hover:bg-[#f5f0e6] transition-all cursor-pointer group"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <p className="font-kalam font-medium text-[#2d2d2d] truncate">{entry.title}</p>
                          {entry.mood && <span className="text-sm flex-shrink-0" title={moodConfig[entry.mood].label}>{moodConfig[entry.mood].emoji}</span>}
                        </div>
                        <p className="text-xs text-[#8a8a8a] font-kalam line-clamp-1">{entry.content}</p>
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-xs text-[#8a8a8a] font-kalam">{format(new Date(entry.date), 'MMM d, yyyy')}</span>
                          {entry.tags.slice(0, 2).map(tag => (
                            <span key={tag} className="text-[10px] bg-[#e8e2d4] text-[#5a5a5a] px-1.5 py-0.5 rounded-full font-kalam">#{tag}</span>
                          ))}
                        </div>
                      </div>
                      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                        <button className="p-1.5 rounded hover:bg-[#e8eef3] text-[#5a5a5a]" onClick={e => { e.stopPropagation(); openEditEntry(entry); }}>
                          <Pencil className="w-3.5 h-3.5" />
                        </button>
                        <button className="p-1.5 rounded hover:bg-[#f5e8e8] text-[#a85a5a]" onClick={e => { e.stopPropagation(); deleteJournalEntry(entry.id); }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-caveat text-2xl flex items-center gap-2">
            <BookOpen className="w-5 h-5 text-[#a99bc4]" /> My Books
          </h2>
          <Button className="journal-btn text-sm" onClick={() => setCreateBookOpen(true)}>
            <Plus className="w-4 h-4 mr-1" /> New Book
          </Button>
        </div>
        <div className="flex gap-4 pb-2 pt-2 -mx-1 px-1" style={{ overflowX: 'auto', overflowY: 'visible' }}>
          {journalBooks.map((book) => {
            const entryCount = journalEntries.filter(e => e.bookId === book.id).length;
            return (
              <motion.div key={book.id} whileHover={{ y: -4 }} className="flex-shrink-0 w-48 cursor-pointer group"
                onClick={() => handleOpenBook(book.id)}>
                <div className="relative rounded-xl overflow-hidden border-2 border-[#e0e0e0] group-hover:border-[#d9b896] transition-all shadow-sm group-hover:shadow-md">
                  <div className="h-32 bg-gradient-to-br from-[#f5ece3] via-[#e8ddd0] to-[#d9ccb8] flex flex-col items-center justify-center p-4 text-center relative">
                    <div className="w-10 h-10 rounded-full overflow-hidden mb-2 border border-[#d9b896] shadow-sm">
                      <img src="/book/book theme icon.png" alt="" className="w-full h-full object-cover" />
                    </div>
                    <h3 className="font-kalam text-sm font-bold text-[#2d2d2d] leading-tight">{book.name}</h3>
                    <p className="font-kalam text-[10px] text-[#5a5a5a] mt-0.5">{book.ownerName}</p>
                  </div>
                  <div className="p-3 bg-[#f9f7f4]">
                    <div className="flex items-center justify-between">
                      <span className="font-kalam text-xs text-[#8a8a8a]">{entryCount} entries</span>
                      <span className="font-kalam text-[10px] text-[#8a8a8a]">{format(new Date(book.startedAt), 'MMM yyyy')}</span>
                    </div>
                  </div>
                  <button className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white/80 p-1 rounded-full hover:bg-[#f5e8e8]"
                    onClick={e => { e.stopPropagation(); if (confirm('Delete this book and all its entries?')) deleteJournalBook(book.id); }}>
                    <Trash2 className="w-3 h-3 text-[#a85a5a]" />
                  </button>
                </div>
              </motion.div>
          ...
          <motion.div whileHover={{ y: -4 }} className="flex-shrink-0 w-48 cursor-pointer" onClick={() => setCreateBookOpen(true)}>
            <div className="h-full min-h-[176px] rounded-xl border-2 border-dashed border-[#d9b896] bg-[#fef9e6]/30 flex flex-col items-center justify-center gap-2 hover:bg-[#fef9e6]/60 transition-all">
              <div className="w-10 h-10 rounded-full bg-[#f5f0e6] border border-[#d9b896] flex items-center justify-center">
                <Plus className="w-5 h-5 text-[#a88a5a]" />
              </div>
              <p className="font-kalam text-sm text-[#a88a5a]">New Book</p>
            </div>
          </motion.div>
        </div>
      </motion.div>
        ...
        <DialogContent className="journal-modal max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-caveat text-2xl flex items-center gap-2">
              {viewingEntry?.mood && <span className="text-xl">{moodConfig[viewingEntry.mood].emoji}</span>}
              {viewingEntry?.title}
            </DialogTitle>
          </DialogHeader>
          {viewingEntry && (
            <div className="space-y-3">
              <div className="flex items-center gap-2 flex-wrap">
                <Badge className="font-kalam text-xs" style={{ backgroundColor: (typeColors[viewingEntry.type] || typeColors.journal).bg, color: (typeColors[viewingEntry.type] || typeColors.journal).text, borderColor: (typeColors[viewingEntry.type] || typeColors.journal).border }}>
                  {viewingEntry.type || 'journal'}
                </Badge>
                <span className="text-xs text-[#8a8a8a] font-kalam">{format(new Date(viewingEntry.date), 'EEEE, MMMM d, yyyy')}</span>
                {viewingEntry.mood && (
                  <span className="font-caveat text-sm" style={{ color: moodConfig[viewingEntry.mood].color }}>
                    {moodConfig[viewingEntry.mood].handwritten}
                  </span>
                )}
              </div>
              <div className="text-sm text-[#2d2d2d] font-kalam whitespace-pre-wrap leading-relaxed bg-[#f9f7f4] rounded-lg p-4 border border-[#e0e0e0]">
                {viewingEntry.content}
              </div>
              {viewingEntry.tags.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {viewingEntry.tags.map(tag => (
                    <span key={tag} className="text-xs bg-[#e8e2d4] text-[#5a5a5a] px-2 py-0.5 rounded-full font-kalam">#{tag}</span>
                  ))}
                </div>
              )}
              <div className="flex gap-2 justify-end pt-2 border-t border-[#e0e0e0]">
                <Button variant="outline" size="sm" className="font-kalam" onClick={() => { openEditEntry(viewingEntry); setViewingEntry(null); }}>
                  <Pencil className="w-3 h-3 mr-1" /> Edit
                </Button>
                <Button variant="outline" size="sm" className="text-[#a85a5a] border-[#d49191] font-kalam hover:bg-[#f5e8e8]"
                  onClick={() => { deleteJournalEntry(viewingEntry.id); setViewingEntry(null); }}>
