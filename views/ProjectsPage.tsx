...
function ProjectForm({ onSubmit, onCancel, initialData }: { 
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({
      title, description, type, status, techStack, partnerId: partnerId || undefined,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
      ...
      <div><label className="font-kalam text-sm font-bold mb-1 block">Assign Partner</label>
...
// ProjectDetailsModal removed in favor of route /projects/[slug]
...
export function ProjectsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-2 space-y-5">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          <AnimatePresence mode="popLayout">
            {filteredProjects.map(project => (
              <ProjectCard key={project.id} project={project} onClick={() => router.push(`/projects/${project.id}`)} onChangeStatus={status => handleChangeStatus(project.id, status)} />
            ))}
          </AnimatePresence>
        </div>
      )}
      ...
      {viewMode === 'kanban' && (
        <KanbanView projects={filteredProjects} onSelect={p => router.push(`/projects/${p.id}`)} onChangeStatus={handleChangeStatus} />
      )}
